...
const  API_BASE_URL = 'https://node-practice-six.vercel.app/api'
@ccclass('NetworkManager')
export class NetworkManager extends Component {

    // 案例1：请求加金币 (POST 请求)
    async addCoin(userId:number){
...
//获取金币排行榜前10名
    ...
    try{
        ...
        if (!response.ok) { //请求成功，但是服务器查不到信息
...
async dailySignIn(userId:number){
    try{
        ...
        if(!response.ok){
            try{
                ...
                return { success: false, message: '网络通了，事没办成' }
            }
            const result  = JSON.parse(text)
            return result
    }catch(e){
            console.log('network-dailysignin-failed')
            return { success: false, message: '网络都没通，搞毛线' } 
        }
    }
...
async addItem(userId:number ,itemId:number, count: number){
    try{
    const response = await fetch(`${API_BASE_URL}/add-item` , {
        method : 'POST',
        headers: {'Content-Type':'application/json'},
        body: JSON.stringify({userId, itemId ,count})
        })
    ...
    async gachaItem(userId: number ){
        try{
            ...
            if(!response.ok){
                ...
                return {success:false, message:'网线通了，事没办成'}
            }
            return JSON.parse(text)
        }catch(error){
            ...
            return {success:false, message:'网都没通，搞毛线'}
        }
    }


     async loginItem(tgId:number , username:string ){
        try{
